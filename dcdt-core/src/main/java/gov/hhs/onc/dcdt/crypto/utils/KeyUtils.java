...
public abstract class KeyUtils {
    private final static int KEY_PAIR_GEN_RAND_SEED_SIZE_DEFAULT = 32;

    public static KeyPairInfo generateKeyPair(KeyPairConfig keyPairConfig) throws CryptographyException {
        return generateKeyPair(keyPairConfig, SecureRandomUtils.getRandom(KEY_PAIR_GEN_RAND_SEED_SIZE_DEFAULT));
    }

    public static KeyPairInfo generateKeyPair(KeyPairConfig keyPairConfig, SecureRandom secureRandom) throws CryptographyException {
        if (!keyPairConfig.hasKeyAlgorithm()) {
            throw new KeyException(String.format("Key pair configuration (class=%s) must include a key algorithm.", ToolClassUtils.getName(keyPairConfig)));
        }

        if (!keyPairConfig.hasKeySize()) {
            throw new KeyException(String.format("Key pair configuration (class=%s) must include a key size.", ToolClassUtils.getName(keyPairConfig)));
        }

        KeyPairGenerator keyPairGen = getKeyPairGenerator(keyPairConfig.getKeyAlgorithm());
        keyPairGen.initialize(keyPairConfig.getKeySize(), secureRandom);

        return new KeyPairInfoImpl(keyPairGen.generateKeyPair());
    }
    ...
    public static KeyFactory getKeyFactory(KeyAlgorithm keyAlg) throws CryptographyException {
        try {
            return CryptographyUtils.PROVIDER_HELPER.createKeyFactory(keyAlg.getName());
        } catch (NoSuchAlgorithmException | NoSuchProviderException e) {
            throw new KeyException(String.format("Unable to get key factory instance for key algorithm (name=%s, providerName=%s).", keyAlg.getName(),
                CryptographyUtils.PROVIDER_NAME), e);
        }
    }
}
